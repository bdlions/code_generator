...
{
    //to add logic connector operator user has to select if from left panel
    if($('#selectable .ui-selected').text().trim() != "if")
    {
        alert("Please select if from left panel to add logical operator.");
        return;
    }

    var total_li_items_left_panel = $("#selectable > li").size();
    var selected_if_starting_spaces = $('#selectable .ui-selected').text().length - $('#selectable .ui-selected').text().trim().length;
    var currentItem = $('#selectable .ui-selected');
    var next_item_starting_space = 0;
    var list_item_counter = 1;
    while(list_item_counter++ < total_li_items_left_panel)
    {
        currentItem = currentItem.next("li");
        next_item_starting_space = currentItem.text().length - currentItem.text().trim().length;
        if(currentItem.text().trim() != "(" && currentItem.text().trim() != ")" && currentItem.text().trim() != "{" && currentItem.text().trim() != "}" && next_item_starting_space <= selected_if_starting_spaces)
        {
            break;
        }
        if(currentItem.text().trim() != "(" && currentItem.text().trim() != ")" && currentItem.text().trim() != "{" && currentItem.text().trim() != "}" && next_item_starting_space > selected_if_starting_spaces)
    ...
    currentItem = $('#selectable .ui-selected');
    list_item_counter = 1;
    ...
    while(list_item_counter++ < total_li_items_left_panel)
    {
        ...
        if(currentItem.text().trim() != "(" && currentItem.text().trim() != ")" && currentItem.text().trim() != "{" && currentItem.text().trim() != "}" && next_item_starting_space > selected_if_starting_spaces)
        {
            //var logical_connector_list_items = "<li class='ui-widget-content'>if condition THEN action</li>";
            var is_condition = 0;
