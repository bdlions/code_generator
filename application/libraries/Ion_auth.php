...
class Ion_auth
{
	...
	{
            $this->ci->ion_auth_model->trigger_events('is_member');
            $member_group = $this->ci->config->item('member_group', 'ion_auth');
            return $this->in_group($member_group);
	}
	...
	{
            $this->ci->ion_auth_model->trigger_events('is_demo');
            $demo_group = $this->ci->config->item('demo_group', 'ion_auth');
            return $this->in_group($demo_group);
	}
	...
	public function in_group($check_group, $id=false)
	{
        ...
        public function resend_activation_email($id, $email)
        {
            $deactivate = $this->ci->ion_auth_model->deactivate($id);
