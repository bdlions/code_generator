...
class Ion_auth_model extends CI_Model {
    ...
    public function login($identity, $password, $remember = FALSE) {
        ...
        $query = $this->db->select('username, email, id, password, active, is_inactivated_by_admin, last_login')
        ...
        if ($query->num_rows() == 1) {
            ...
            if ($user->password === $password) {
                ...
                //storing session id
                $this->db->update($this->tables['users'], array('session_id' => $this->session->userdata('session_id')), array('id' => $user->id));
    ...
    public function users() {
        ...
        if (isset($this->_ion_or_like)) {
            foreach ($this->_ion_or_like as $or_like) {
                $this->db->or_like($or_like);
            }

            $this->_ion_or_like = array();
        }
